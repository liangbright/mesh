...
from TriangleMesh import Tri3Mesh as TriangleMesh
#%%
class Quad4Mesh(PolygonMesh):
    ...
    def update_element_area_and_normal(self):
         self.element_area, self.element_normal=Quad4Mesh.cal_element_area_and_normal(self.node, self.element)
    ...
    def subdivide_elements(self):
         self.node, self.element = Quad4Mesh.subdivide(self.node, self.element)
    ...
    def to_tri3_mesh(self):
        #return a triangle mesh: mesh_new
        #mesh_new.node is mesh.node
        element_new=[]
        for m in range(0, self.element.shape[0]):
            # x3------x2
            # |       |
...
if __name__ == "__main__":
    ...
    #%%
    wall_tri=wall.to_tri3_mesh()
    wall_tri.save_by_vtk("wall_tri.vtk")
