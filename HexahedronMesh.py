    ...
    def build_adj_node_link(self, undirected):
        ...
        for m in range(0, len(self.element)):
            ...
            if undirected == True:
                ...
                if id4 < id5:
                    adj_node_link.append([id4, id5])
                else:
                    adj_node_link.append([id5, id4])
                if id5 < id6:
                    adj_node_link.append([id5, id6])
                else:
                    adj_node_link.append([id6, id5])
                if id6 < id7:
                    adj_node_link.append([id6, id7])
                else:
                    adj_node_link.append([id7, id6])
                if id7 < id4:
                    adj_node_link.append([id7, id4])
                else:
                    adj_node_link.append([id4, id7])
                if id0 < id4:
                    adj_node_link.append([id0, id4])
                else:
                    adj_node_link.append([id4, id0])
                if id1 < id5:
                    adj_node_link.append([id1, id5])
                else:
                    adj_node_link.append([id5, id1])
                if id2 < id6:
                    adj_node_link.append([id2, id6])
                else:
                    adj_node_link.append([id6, id2])
                if id3 < id7:
                    adj_node_link.append([id3, id7])
                else:
                    adj_node_link.append([id7, id3])
